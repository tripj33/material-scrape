...
const MEMORY_LIMITS = {
  ...
  PAGE_TIMEOUT: 90000,        // Longer for login flows
  NAVIGATION_TIMEOUT: 90000,   // Longer for login flows
  ...
  REQUEST_TIMEOUT: 120000     // 2 minutes for login flows
};
  ...
  function createPagePool() {
    pagePool = genericPool.createPool({
      create: async () => {
        ...
        // Less aggressive resource blocking to support login flows
        await page.setRequestInterception(true);
        page.on('request', (request) => {
          const resourceType = request.resourceType();
          // Allow more resources for login forms to work properly
